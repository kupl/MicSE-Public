...
let test_result :
   ...
   let (_, (res : res)) = ExecFlow_s.coop_prover_refuter_toss arg in
...
let test_list : unit -> OUnit2.test =
  ...
  OUnit2.test_list
    [
      gen_basic_test ~name:"example1"
        ~file_path:
          ( "../benchmarks/motivating_examples/example1.tz",
            "../benchmarks/motivating_examples/example1.storage.tz"
          )
        ~timeout:300 ~result:(1, 2, 0);
    ]
