...
def collect_files():
    ...
    tzlist = list(filter(lambda x: x.endswith(".tz")
                  and not x.endswith(".storage.tz"), filelist))
...
def run_multi():
    ...
    raw_job = [(idx, target, res, syncdata)
...
def run_each(idx, target, res, syncdata):
    ...
    try:
        timeout = None if NON_TIMEOUT else TOTAL_TIMEOUT*1.1
        subprocess.run(job['cmd'], stdout=tmp_fp, stderr=subprocess.STDOUT,
    ...
    with open(job['output'], "w") as fp:
        ...
        fp.write("...{}s [{}/{}]\n".format(job['run_time'],
                 syncdata[0], syncdata[1]))
    LOG.info("job #{} is done in {}s [{}/{}].".format(idx,
             job['run_time'], syncdata[0], syncdata[1]))
    res.append(job)
