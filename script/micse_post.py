        ...
        if not SLACK_POST:
            ...
            with open(job['output'], "r") as fp:
                self.client.files_upload(
                    ...
                    initial_comment="Job #{} for `{}`".format(
                        job['idx'], job['target']),
                    filename="{}_{}_{}.out".format(INPUT_BENCHMARK, job['idx'], time.strftime(
                        "%Y-%m-%d %H:%M:%S", time.gmtime(SCRIPT_START_TIME))),
                    filetype="text",
