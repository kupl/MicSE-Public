...
type buyTicketParameter is nat
...
type storage is record [
    ...
    ticket_price_map: big_map (address, tez);
...
function buy_ticket (const param: nat; var store: storage): returnType is {
    if store.raffle_is_open then {
        const ticket_price : tez = param * store.jackpot;
        ...
        then failwith("The sender did not send the right tez amount.")
            ...
            then failwith("Each player can participate only once.")
                ...
                store.ticket_price_map[current_player] := ticket_price;
            }
        }
    } else {
        ...
        if store.raffle_is_open then {
            if Tezos.get_now() < store.close_date then failwith("The raffle must remain open for at least 7 days.")
                ...
                then failwith("The hash does not match the hash of the winning ticket.")
                    ...
                    const award: tez =
                        case (store.ticket_price_map[winner]) of [
                        | Some(a) -> a
                    ...
                    const op: operation = Tezos.transaction(unit, award, receiver);
                    operations := list [ op; ];
